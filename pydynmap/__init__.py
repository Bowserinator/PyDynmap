...
class Dynmap(object):
    ...
    def getClaims(self):  # Gets all claims
        ...
        for key in claims:
            claim = claims[key]
            desc = claim["desc"]
            claimType = re.findall(
                '<div class="regioninfo"> <strong>= (.*) =</strong><br>', desc)[0]
            claimID = key.split("_")[-1]
            claimTeleport = "/tp " + \
                re.findall('<br> /tp (.*)<br><br>', desc)[0].split("<br>")[0]
            claimOwner = claim["label"]
            claimPermTrust = re.findall('strong>Permission Trust:</strong><br>(.*)<br>', desc)[
                0].lstrip().replace("<br>", "").replace("<br/>", "").split(', ')
            claimTrust = re.findall('strong>Trust:</strong><br>(.*)<br>', desc)[
                0].lstrip().replace("<br>", "").replace("<br/>", "").split(', ')
            claimAccessTrust = re.findall('strong>Access Trust:</strong><br>(.*)<br>', desc)[
                0].lstrip().replace("<br/>", "").replace("<br>", "").split(', ')
            claimContainerTrust = re.findall('strong>Container Trust:</strong><br>(.*)<br>', desc)[
                0].lstrip().replace("<br/>", "").replace("<br>", "").split(', ')
